...
import AmbulanceLocation from '@/app/ui/ambulance/AmbulanceLocation'; // Adjust path if needed
...
const AmbulanceDispatchedPage = () => {
  ...
  const ambulanceId = localStorage.getItem('ambulanceId') || "defaultAmbulanceId";
    ...
    <div className="flex flex-col items-center justify-center p-4 text-white border-b-2 rounded-xl">
      ...
      {ambulanceDetails && (
        <div className="mb-4 text-left w-full max-md:text-sm">
          ...
          <p className=''> Platenumber: {ambulanceDetails.plateNumber}</p>
          <p>Drver Name: {ambulanceDetails.driverName}</p>
          <p>Drver Contact: {ambulanceDetails.driverContact}</p>

          {/* Render other ambulance details here */}
        </div>
      )}

<button
        onClick={openModal}
        disabled={loadingDetails === true || !!errorDetails || ambulanceDetails === null}
        className={`mb-4 bg-blue-500 hover:bg-blue-700 focus:ring-2 focus:ring-blue-500 focus:ring-offset-2 max-md:text-sm shadow-2xl text-white font-bold py-2 px-4 rounded transition-all duration-300 ease-in-out ${
          loadingDetails === true || !!errorDetails || ambulanceDetails === null
